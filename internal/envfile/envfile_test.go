...
func TestParseEnv_BasicKeyValue(t *testing.T) {
	dir := t.TempDir()
	path := filepath.Join(dir, ".env")
	os.WriteFile(path, []byte("DB_HOST=127.0.0.1\nDB_PORT=3380\n"), 0o644)

	got, err := envfile.ParseEnv(path)
	if err != nil {
		t.Fatal(err)
	}
...
func TestParseEnv_SkipsCommentsAndBlanks(t *testing.T) {
	...
	if _, ok := got["# Generated by dev"]; ok {
	...
	if got["APP_KEY"] != "abc" {
		t.Errorf("APP_KEY: got %q", got["APP_KEY"])
	}
	if len(got) != 2 {
...
func TestParseEnv_ValueWithEquals(t *testing.T) {
	dir := t.TempDir()
	path := filepath.Join(dir, ".env")
	os.WriteFile(path, []byte("APP_KEY=base64:abc==\n"), 0o644)

	got, err := envfile.ParseEnv(path)
	if err != nil {
		t.Fatal(err)
	}
	if got["APP_KEY"] != "base64:abc==" {
		t.Errorf("APP_KEY: got %q want base64:abc==", got["APP_KEY"])
	}
}

func TestParseEnv_FileNotExist(t *testing.T) {
	_, err := envfile.ParseEnv(filepath.Join(t.TempDir(), "nonexistent.env"))
	if !os.IsNotExist(err) {
		t.Errorf("expected os.ErrNotExist, got %v", err)
	}
}

func TestParseEnv_EmptyFile(t *testing.T) {
	dir := t.TempDir()
	path := filepath.Join(dir, ".env")
...
func TestDiff_NewKeys(t *testing.T) {
	generated := map[string]string{"DB_HOST": "127.0.0.1", "DB_PORT": "3380"}
	existing := map[string]string{"DB_HOST": "127.0.0.1"}

	newKeys, changedKeys := envfile.Diff(generated, existing)

	if len(newKeys) != 1 || newKeys[0] != "DB_PORT" {
		t.Errorf("newKeys: got %v want [DB_PORT]", newKeys)
	}
	if len(changedKeys) != 0 {
		t.Errorf("changedKeys: got %v want []", changedKeys)
	}
}

func TestDiff_ChangedKeys(t *testing.T) {
	generated := map[string]string{"DB_HOST": "127.0.0.1", "DB_PORT": "3380"}
	existing := map[string]string{"DB_HOST": "127.0.0.1", "DB_PORT": "3300"}

	newKeys, changedKeys := envfile.Diff(generated, existing)

	if len(newKeys) != 0 {
		t.Errorf("newKeys: got %v want []", newKeys)
	}
	if len(changedKeys) != 1 || changedKeys[0] != "DB_PORT" {
		t.Errorf("changedKeys: got %v want [DB_PORT]", changedKeys)
	}
}

func TestDiff_NoDiff(t *testing.T) {
	generated := map[string]string{"DB_HOST": "127.0.0.1", "DB_PORT": "3380"}
	existing := map[string]string{"DB_HOST": "127.0.0.1", "DB_PORT": "3380", "APP_KEY": "abc"}

	newKeys, changedKeys := envfile.Diff(generated, existing)

	if len(newKeys) != 0 || len(changedKeys) != 0 {
		t.Errorf("expected no diff, got new=%v changed=%v", newKeys, changedKeys)
	}
}

// --- UserVars tests ---

func TestUserVars_ReturnsNonGeneratedKeys(t *testing.T) {
	generated := map[string]string{"DB_HOST": "127.0.0.1", "DB_PORT": "3380"}
	...
	user := envfile.UserVars(generated, existing)

	if _, ok := user["DB_HOST"]; ok {
		t.Error("DB_HOST should not be in user vars")
	}
	if user["APP_KEY"] != "base64:abc" {
		t.Errorf("APP_KEY: got %q", user["APP_KEY"])
	}
	if user["APP_ENV"] != "local" {
		t.Errorf("APP_ENV: got %q", user["APP_ENV"])
	}
	if len(user) != 2 {
		t.Errorf("expected 2 user vars, got %d", len(user))
	}
}

func TestUserVars_EmptyWhenNoUserVars(t *testing.T) {
	generated := map[string]string{"DB_HOST": "127.0.0.1"}
	existing := map[string]string{"DB_HOST": "127.0.0.1"}

	user := envfile.UserVars(generated, existing)
	if len(user) != 0 {
		t.Errorf("expected empty user vars, got %v", user)
	}
}

// --- WriteMerged tests ---

func TestWriteMerged_TwoSections(t *testing.T) {
	dir := t.TempDir()
	path := filepath.Join(dir, ".env")

	devVars := map[string]string{"DB_HOST": "127.0.0.1", "DB_PORT": "3380"}
	userVars := map[string]string{"APP_KEY": "abc", "APP_ENV": "local"}

	if err := envfile.WriteMerged(path, devVars, userVars); err != nil {
		t.Fatal(err)
	}
	...
	if !strings.Contains(content, "DB_HOST=127.0.0.1") {
		t.Errorf("missing dev var DB_HOST\n%s", content)
	}
	if !strings.Contains(content, "APP_KEY=abc") {
		t.Errorf("missing user var APP_KEY\n%s", content)
	}
	// Dev section before user section.
	dbIdx := strings.Index(content, "DB_HOST")
	appIdx := strings.Index(content, "APP_KEY")
	if dbIdx >= appIdx {
		t.Errorf("dev vars should appear before user vars\n%s", content)
	}
}

func TestWriteMerged_NoUserVars(t *testing.T) {
	dir := t.TempDir()
	path := filepath.Join(dir, ".env")

	devVars := map[string]string{"DB_HOST": "127.0.0.1"}
	if err := envfile.WriteMerged(path, devVars, nil); err != nil {
		t.Fatal(err)
	}
	data, _ := os.ReadFile(path)
	if !strings.Contains(string(data), "DB_HOST=127.0.0.1") {
		t.Errorf("missing dev var\n%s", string(data))
	}
}

func TestWriteMerged_DevVarsAlphabetical(t *testing.T) {
	dir := t.TempDir()
	path := filepath.Join(dir, ".env")

	devVars := map[string]string{"Z_VAR": "z", "A_VAR": "a", "M_VAR": "m"}
	if err := envfile.WriteMerged(path, devVars, nil); err != nil {
		t.Fatal(err)
	}
	data, _ := os.ReadFile(path)
	content := string(data)
	if strings.Index(content, "A_VAR") >= strings.Index(content, "M_VAR") ||
		strings.Index(content, "M_VAR") >= strings.Index(content, "Z_VAR") {
		t.Errorf("dev vars not alphabetical\n%s", content)
	}
}
