...
// ParseEnv reads a .env file and returns all key=value pairs.
...
// Diff compares generated vars against an existing .env map.
// newKeys: keys in generated absent from existing.
// changedKeys: keys in both but with different values.
func Diff(generated, existing map[string]string) (newKeys, changedKeys []string) {
...
// UserVars returns keys from existing that are not present in generated.
func UserVars(generated, existing map[string]string) map[string]string {
	user := map[string]string{}
	for k, v := range existing {
		if _, managed := generated[k]; !managed {
			user[k] = v
		}
	}
	return user
...
// PrintDiff prints a human-readable summary of what will change.
func PrintDiff(newKeys, changedKeys []string, generated, existing map[string]string) {
	fmt.Println("Changes to .env:")
	for _, k := range newKeys {
		fmt.Printf("  + %s=%s  (new)\n", k, generated[k])
	}
	for _, k := range changedKeys {
		fmt.Printf("  ~ %-24s  %s → %s\n", k, existing[k], generated[k])
	}
}

// WriteMerged writes devVars (sorted, under a managed-section header) followed
// by userVars (under a user-defined header). Overwrites path.
func WriteMerged(path string, devVars, userVars map[string]string) error {
	devKeys := make([]string, 0, len(devVars))
	for k := range devVars {
	...
	var sb strings.Builder
	sb.WriteString("# Generated by dev — do not edit this section\n")
	for _, k := range devKeys {
		sb.WriteString(k)
		sb.WriteByte('=')
		sb.WriteString(devVars[k])
		sb.WriteByte('\n')
	}

	if len(userVars) > 0 {
		userKeys := make([]string, 0, len(userVars))
		...
		sb.WriteByte('\n')
		sb.WriteString("# User-defined variables\n")
		for _, k := range userKeys {
			sb.WriteString(k)
			sb.WriteByte('=')
			sb.WriteString(userVars[k])
			sb.WriteByte('\n')
		}
	}
